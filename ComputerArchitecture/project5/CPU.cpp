...
void CPU::decode() {
  ...
  funct = instr & 0x3f;
  uimm = instr & 0xffff;
  simm = (instr & 0xffff) - ((instr & 0x8000) << 1);
  addr = instr & 0x03ffffff;

  opIsLoad = false, opIsStore = false, opIsMultDiv = false, writeDest = false;
  aluSrc1 = regFile[REG_ZERO], aluSrc2 = regFile[REG_ZERO], storeData = 0;
  ...
  switch(opcode) {
    ...
    case 0x1a: D(cout << "trap " << hex << addr);
               switch(addr & 0xf) {
                 ...
                 case 0x1: stats.registerSrc(rs, EXE1);
    ...
    case 0x23: D(cout << "lw " << regNames[rt] << ", " << dec << simm << "(" << regNames[rs] << ")");
               opIsLoad = true; writeDest = true; destReg = rt; stats.registerDest(rt, WB);
	       aluOp = ADD;
	       aluSrc1 = regFile[rs]; stats.registerSrc(rs, EXE1);
	       aluSrc2 = simm;
	       stats.countMemOp();
    ...
    case 0x2b: D(cout << "sw " << regNames[rt] << ", " << dec << simm << "(" << regNames[rs] << ")");
               ...
               aluOp = ADD; 
               aluSrc1 =regFile[rs]; stats.registerSrc(rs, EXE1); stats.registerSrc(rt, MEM1);
               aluSrc2 = simm;
	       stats.countMemOp();
...
void CPU::printFinalStats() {
  ...
  cout << "CPI: " << setprecision(2) << fixed << (1.0*stats.getCycles())/(1.0*instructions) << endl << endl;
