...
void CPU::decode() {
  ...
  switch(opcode) {
    case 0x00:
      switch(funct) {
        ...
        case 0x08: D(cout << "jr " << regNames[rs]);
                   pc = regFile[rs]; stats.registerSrc(rs);    // set the next pc location to the address in rs
                   stats.flush(2);
                   break;
        ...
        case 0x12: D(cout << "mflo " << regNames[rd]);
                   writeDest = true; destReg = rd; stats.registerDest(destReg);
                   aluOp = ADD;
    ...
    case 0x04: { D(cout << "beq " << regNames[rs] << ", " << regNames[rt] << ", " << pc + (simm << 2));
               ...
               uint32_t saved_pc; 
               saved_pc = (pc - 4);
               prediction = branchPred.predict((pc - 4), target);
               if(regFile[rs] == regFile[rt])
               {
                   pc = pc + (simm << 2);
                   stats.countTaken();
                   if(branchPred.isMispredict(prediction, target, true, pc)) {
                      stats.flush(2);
                   }
                   prediction = true;
                  ...
                  if(branchPred.isMispredict(prediction, pc, false, pc)) {
                      stats.flush(2);
                  }
                  prediction = false;
               }
               branchPred.update(saved_pc, prediction, pc);
               break;
    }
